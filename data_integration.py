...
DBSCAN_PCA_FILENAME = Path(__file__).parent / 'data' / 'anomalies_dbscan_pca.csv'
ISOLATION_ANOMALIES_FILENAME = Path(__file__).parent / 'data' / 'all_stock_anomalies_5th_threshold.csv'
DBSCAN_NONPCA_FILENAME = Path(__file__).parent / 'data' / 'DBSCAN-nonpca_labels.csv'
...
dbscan_anomalies = pd.read_csv(DBSCAN_PCA_FILENAME)
isolation_anomalies = pd.read_csv(ISOLATION_ANOMALIES_FILENAME)
dbscan_nonpca_anomalies = pd.read_csv(DBSCAN_NONPCA_FILENAME)
...
# Add 'dbscan_match' column (PCA): 1 if match, 0 otherwise
overall_anomalies['dbscan_pca'] = overall_anomalies.merge(
    dbscan_anomalies[['Ticker', 'Date']], on=['Ticker', 'Date'], how='left', indicator=True
)['_merge'].eq('both').astype(int)

# Add 'dbscan_nonpca_match' column: 1 if match, 0 otherwise
overall_anomalies['dbscan_nonpca'] = overall_anomalies.merge(
    dbscan_nonpca_anomalies[['Ticker', 'Date']], on=['Ticker', 'Date'], how='left', indicator=True
)['_merge'].eq('both').astype(int)
